...
class Decoder(nn.Module):
    ...
    def assign_adain_params(self, adain_params, model):
        ...
        for m in model.modules():
            if m.__class__.__name__ == "AdaptiveInstanceNorm2d":
                ...
                m.bias = mean.contiguous().view(-1)#contiguous()确保mean和std的内存连续，view(-1)将mean和std调整为一维张量
    ...
    def forward(self,c,s):
        random_code = torch.rand(1, 8).cuda()
        s_merge = s + random_code
        adain_params=self.mlp(s_merge)#s为风格编码 s={[Tensor:(1,8)]}
